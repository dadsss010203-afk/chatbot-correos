...
def buscar(pregunta: str, n_resultados: int = None) -> str:
    ...
    Args:
        ...
        n_resultados: cuántos chunks devolver (por defecto N_RESULTADOS del .env)

    Returns:
        Contexto como string listo para inyectar en el prompt de Ollama.
        Limitado a 800 chars para no saturar el modelo.
    """
    col = get_collection()
    n   = n_resultados or N_RESULTADOS

    results  = col.query(query_texts=[pregunta], n_results=n)
    contexto = "\n\n".join(results["documents"][0])

    # Limitar a 800 chars sin cortar palabras
    if len(contexto) > 800:
